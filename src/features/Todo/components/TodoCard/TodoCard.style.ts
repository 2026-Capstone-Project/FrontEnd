...
export const Wrapper = styled.div<{
  ...
  box-shadow: ${({ $isOverdue }) =>
    $isOverdue ? '0 0 2px 0 rgba(0, 0, 0, 0.10)' : '0 0 2px 0 rgba(0, 0, 0, 0.1)'};
  box-sizing: border-box;
  border: ${({ $isEditing }) => ($isEditing ? `1.5px solid` : 'none')};
  border-color: ${({ $isEditing }) => ($isEditing ? `${theme.colors.textColor3}` : 'none')};
  background: ${({ $isOverdue, $isHighlight }) => {
    if ($isOverdue) {
      return 'linear-gradient(90deg, #FFE9E8 0%, rgba(251, 226, 225, 0.04) 100%), #FFF'
    }
    if ($isHighlight) {
      return 'linear-gradient(90deg, #e9f4f7 0%, rgba(233, 244, 247, 0) 100%), #fff'
    }
    return 'white'
  }};
  display: flex;
